...
CREATE TABLE IF NOT EXISTS `Detect`.`Supervisors` (
 ...
 `SupervidorCompany` INT NOT NULL,
 `SupervisorPosition` INT NOT NULL,
 PRIMARY KEY (`SupervisorId`),
 
 INDEX `SupervidorCompany_idx` (`SupervidorCompany` ASC),
 INDEX `SupervisorPosition_idx` (`SupervisorPosition` ASC),
 
 CONSTRAINT `SupervidorCompany`
 FOREIGN KEY (`SupervidorCompany`)
...
CREATE TABLE IF NOT EXISTS `Detect`.`Platforms` (
 ...
 CONSTRAINT `ApiFeatures`
 FOREIGN KEY (`ApiFeatures`)
 REFERENCES `Detect`.`FeatureTypes` (`FeatureTypeId`)
 ON DELETE RESTRICT
 ON UPDATE RESTRICT
 );
