...
void * staff(void *arg){
	...
	while(1){
		pthread_mutex_lock(&mutex);
		staff_waiting++;
		while(cars_parked >= N_PARKING_SPOTS && prof_waiting > 0){
			pthread_cond_wait(&p_line, &mutex);
		}
		parking(id);
		cars_parked++;
...
int main(int argc, char** argv){
	...
	for(int i=0; i<(N_PROF+N_STUD+N_STAFF); i++){
		...
		*id = i;
		if(i%3 == 0)
			pthread_create(&cars[i], NULL, professor, (void *) (id));
		if(i%3 == 1)
			pthread_create(&cars[i], NULL, staff, (void *) (id));
		if(i%3 == 2)
			pthread_create(&cars[i], NULL, student, (void *) (id));
